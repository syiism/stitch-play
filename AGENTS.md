...
- **单步退出 + 异 id 定位**：`collExit` 单步即可把当前集完全替换主队列槽位并保留进度（无需二次退出），合集标记 `exited` 保留尾巴（尾巴耗尽/切走时以 `tailConsumed` 销毁并清快照）；进入合集的起播定位按 `videoId`（规则 2A/2B）而非盲写 EP1，以兼容主队列卡 id（沐凡=裸 `vid`，即首集 item_id；其他源 `drama-*`）与分集 id（`ep-*`）不同的体系，锚点比对剥 `ep-` 前缀归一（见 v1.2 §05/§06）。
